...
{
    /// <summary>
    /// Theme settings for the default UGUI bug-report form. Pass an
    /// instance to <see cref="BugReportForm.OpenAsync"/> to override
    ...
    /// </summary>
    public sealed class BugReportFormTheme
    {
        ...
        public string? Title { get; set; } = "Report a bug";

        /// <summary>Label above the single-line title field. Override to localize.</summary>
        public string TitleFieldLabel { get; set; } = "What went wrong?";

        /// <summary>Placeholder inside the title field. Override to localize.</summary>
        public string TitlePlaceholder { get; set; } = "Short summary";

        /// <summary>Label above the multi-line description field. Override to localize.</summary>
        public string DescriptionFieldLabel { get; set; } = "Details";

        /// <summary>Placeholder inside the description field. Override to localize.</summary>
        public string DescriptionPlaceholder { get; set; } = "What happened, and how can we reproduce it?";

        /// <summary>Label above the severity selector. Override to localize.</summary>
        public string SeverityFieldLabel { get; set; } = "How bad is it?";

        /// <summary>
        /// Player-facing labels for the four severity levels, in
        /// low → critical order. Override to localize; the values sent to
        /// Playloop are always the fixed slugs
        /// (<see cref="BugReportSeverities"/>), so localization never
        /// affects how reports read on the dashboard.
        /// </summary>
        public string[] SeverityLabels { get; set; } = { "Low", "Medium", "High", "Critical" };
        ...
        /// (buttons, severity cells, the close button, the branding badge).
        ...
        /// with <see cref="BugReportForm.IsOpen"/> or the
        /// <see cref="BugReportForm.Opened"/> / <see cref="BugReportForm.Closed"/>
        /// events.
        /// </summary>
    ...
    /// <summary>
    /// Outcome of a <see cref="BugReportForm.OpenAsync"/> call:
    /// <c>Submitted</c> when the player clicked Submit (and the request
    /// landed at Playloop), <c>Dismissed</c> when they clicked Cancel
    /// or hit Escape, <c>Failed</c> when validation or the network
    /// rejected the submission.
    /// </summary>
    public enum BugReportFormOutcome
    {
        Submitted,
        Dismissed,
        Failed,
    ...
    /// <summary>
    /// Return type for <see cref="BugReportForm.OpenAsync"/>. When
    /// <see cref="Outcome"/> is <see cref="BugReportFormOutcome.Submitted"/>,
    /// <see cref="Result"/> carries the server response;
    /// <see cref="Error"/> is populated only on
    /// <see cref="BugReportFormOutcome.Failed"/>.
    /// </summary>
    public sealed class BugReportFormResult
    {
        public BugReportFormOutcome Outcome { get; init; }
        public SubmitBugReportResult? Result { get; init; }
        public string? Error { get; init; }
    }

    /// <summary>
    /// Programmatic UGUI default form for Bug Reports. Builds a
    /// Canvas + VerticalLayoutGroup form at runtime, awaits the
    /// player's title / description / severity, and submits through
    /// <see cref="BugReportApi"/>.
    ///
    ...
    /// <see cref="BugReportApi.SubmitAsync"/> from their own overlay.
    ///
    ...
    /// <see cref="BugReportFormOutcome.Failed"/> rather than spawning a
    /// second overlay that would fight over the EventSystem and Canvas.
    /// </summary>
    public sealed class BugReportForm
    {
        private readonly BugReportApi _api;
        ...
        // Single-overlay guard, shared across every BugReportForm instance:
        // a live form may spawn and own the scene's EventSystem, so two
        // open forms (e.g. bug report + a feedback prompt) would fight over
        // input focus and Canvas sorting. 0 = none open, 1 = one open. Held
        // for the form's whole lifetime, released in OpenAsync's finally.
        ...
        public BugReportForm(BugReportApi api, Telemetry.EventConfigStore? eventConfig = null)
        {
        ...
        /// <summary>
        /// Open the default UGUI bug-report form against a concrete session
        ...
        ///
        /// Pass <paramref name="sessionId"/> to attribute the report to a
        /// known session; the form still submits with no session when the
        /// game can't resolve one.
        /// </summary>
        public Task<BugReportFormResult> OpenAsync(
            string? sessionId = null,
            BugReportFormTheme? theme = null,
            CancellationToken ct = default)
        {
            return OpenAsync(() => sessionId, theme, ct);
        }

        /// <summary>
        /// Open the default UGUI bug-report form, resolving the session id
        /// lazily when the player clicks Submit. Use this overload to prompt
        ...
        /// at session start without blocking on the network. The provider
        /// is read once, when the player submits; a bug report is still
        /// filed (unattributed) if it yields no id.
        ///
        /// Only one form may be open at a time across the whole SDK (the
        /// overlay owns the EventSystem). A second concurrent open resolves
        /// immediately as <see cref="BugReportFormOutcome.Failed"/> without
        /// spawning a competing overlay.
        /// </summary>
        public async Task<BugReportFormResult> OpenAsync(
            ...
            CancellationToken ct = default)
        {
            ...
            if (!_api.Enabled)
            {
                return new BugReportFormResult
                {
                    Outcome = BugReportFormOutcome.Failed,
                    Error = "Playloop is not configured (no ingest key); bug-report form is disabled.",
                };
            }

            var resolvedTheme = theme ?? new BugReportFormTheme();
            ...
            if (Interlocked.CompareExchange(ref _openGuard, 1, 0) != 0)
            {
                ...
                {
                    Outcome = BugReportFormOutcome.Failed,
                    Error = "A bug-report form is already open.",
                };
            }
            ...
            {
                FireSafely(Opened);
                var tcs = new TaskCompletionSource<BugReportFormOutcomeRaw>();
                ...
                var rootGo = BuildForm(resolvedTheme, tcs, showBranding);
                ...
                if (ct.CanBeCanceled)
                {
                    ctReg = ct.Register(() =>
                    {
                        ...
                        tcs.TrySetResult(new BugReportFormOutcomeRaw { Outcome = BugReportFormOutcome.Dismissed });
                    });
                }

                BugReportFormOutcomeRaw raw;
                try
                {
                ...
                if (raw.Outcome == BugReportFormOutcome.Dismissed)
                {
                    return new BugReportFormResult { Outcome = BugReportFormOutcome.Dismissed };
                }

                // Resolve the session id now, after the player filled the
                // form. Unlike feedback, a bug report is still worth filing
                // without a session, so a null id doesn't fail the submit -
                // it just files an unattributed report.
                var sessionId = sessionIdProvider();

                // Submit phase. The form already validated title +
                // description before letting the player click Submit, so any
                // failure here is a server-side reject (rate limit, auth).
                try
                {
                    ...
                    return new BugReportFormResult
                    {
                        Outcome = BugReportFormOutcome.Submitted,
                        Result = result,
                    };
                }
        ...
        private struct BugReportFormOutcomeRaw
        {
            public BugReportFormOutcome Outcome;
            public string? Title;
        ...
        private static GameObject BuildForm(
            BugReportFormTheme theme,
            TaskCompletionSource<BugReportFormOutcomeRaw> tcs,
            bool showBranding)
        {
            ...
            var root = new GameObject("Playloop_BugReportForm");
...
#endif
            {
                var es = new GameObject("Playloop_BugReportEventSystem");
            ...
            System.Action dismiss = () => tcs.TrySetResult(new BugReportFormOutcomeRaw
            {
                Outcome = BugReportFormOutcome.Dismissed,
            });
            ...
            AddLabel(panel.transform, theme.TitleFieldLabel, theme, isHeading: false);
            var titleWidget = BuildText(panel.transform, theme.TitlePlaceholder, theme, multiline: false);

            // Description field (multi-line).
            AddLabel(panel.transform, theme.DescriptionFieldLabel, theme, isHeading: false);
            var descriptionWidget = BuildText(panel.transform, theme.DescriptionPlaceholder, theme, multiline: true);

            // Severity selector (Low / Medium / High / Critical, default Medium).
            AddLabel(panel.transform, theme.SeverityFieldLabel, theme, isHeading: false);
            var severityWidget = BuildSeverity(panel.transform, theme);

            // Buttons row: Cancel + Submit. Cancel resolves as Dismissed
            // without attempting validation; Submit validates the required
            // title + description and resolves Submitted when both are set.
            ...
            BuildButton(buttonRow.transform, theme.SubmitLabel,
                theme.ButtonPrimaryBg, theme.ButtonPrimaryText, theme,
                onClick: () =>
                {
                    var title = titleWidget.ReadValue();
                    var description = descriptionWidget.ReadValue();

                    // Surface the missing-required state inline. Don't
                    // resolve the task. Let the player fill it in and click
                    // Submit again.
                    var missing = false;
                    if (string.IsNullOrEmpty(title)) { titleWidget.SetError(true); missing = true; }
                    else titleWidget.SetError(false);
                    if (string.IsNullOrEmpty(description)) { descriptionWidget.SetError(true); missing = true; }
                    else descriptionWidget.SetError(false);
                    if (missing) return;

                    tcs.TrySetResult(new BugReportFormOutcomeRaw
                    {
                        Outcome = BugReportFormOutcome.Submitted,
                        Title = title,
                        Description = description,
                        Severity = severityWidget.ReadValue(),
                    });
                });
        ...
        private sealed class SeverityWidget : IFieldWidget
        {
            // Seeded to the default so a player who never touches the row
            // still submits Medium.
            public string Value = BugReportSeverities.Default;
            ...
            public void SetError(bool show) { /* severity always has a default; nothing to flag. */ }
            ...
            public void Select(int index)
            {
                ...
                {
                    Cells[i].color = (i == index) ? SelectedBg : DefaultBg;
                }
            }
        }

        private static IFieldWidget BuildSeverity(Transform parent, BugReportFormTheme theme)
        {
            var widget = new SeverityWidget
            {
                SelectedBg = theme.AccentColor,
                DefaultBg = theme.InputBgColor,
            };

            var row = new GameObject("Severity", typeof(RectTransform));
            ...
            // Fixed slugs from the API; labels come from the theme (localizable)
            // but fall back to the slug order if a caller supplies too few.
            var slugs = BugReportSeverities.All;
            var cells = new List<Image>(slugs.Count);
            var values = new List<string>(slugs.Count);
            int defaultIndex = 0;
            for (int i = 0; i < slugs.Count; i++)
            {
                ...
                var slug = slugs[i];
                values.Add(slug);
                if (string.Equals(slug, BugReportSeverities.Default, System.StringComparison.Ordinal))
                    defaultIndex = i;

                var label = (theme.SeverityLabels != null && i < theme.SeverityLabels.Length)
                    ? theme.SeverityLabels[i]
                    : slug;

                var cell = AddImage(row.transform, $"sev{i}", theme.InputBgColor);
                cell.AddComponent<LayoutElement>().minHeight = 36f;
                var btn = cell.AddComponent<Button>();
                btn.targetGraphic = cell.GetComponent<Image>();
                MakeHoverable(cell, theme);
                AddCenteredLabel(cell.transform, label, theme);
                btn.onClick.AddListener(() => widget.Select(captured));
                cells.Add(cell.GetComponent<Image>());
            }
            widget.Cells = cells.ToArray();
            widget.Values = values.ToArray();
            widget.Select(defaultIndex);
            return widget;
        }
        ...
        private static IFieldWidget BuildText(
            Transform parent, string placeholderText, BugReportFormTheme theme, bool multiline)
        {
            ...
            // Enforce the server's field caps client-side so the player can't
            // type past them and eat a 400.
            input.characterLimit = multiline ? 8000 : 200;
        ...
        private static Texture2D? GetHandCursorTexture(out Vector2 hotspot)
        {
            ...
            hotspot = new Vector2(
                Playloop.Feedback.PlayloopHandCursor.HotspotX * scale,
                Playloop.Feedback.PlayloopHandCursor.HotspotY * scale);
            if (_handCursorTex != null && _handCursorScale == scale) return _handCursorTex;
            if (_handCursorLoadFailed) return null;
            try
            {
                var bytes = System.Convert.FromBase64String(Playloop.Feedback.PlayloopHandCursor.PngBase64);
        ...
        private static void MakeHoverable(
            GameObject go, BugReportFormTheme theme,
        ...
        private static void AddCloseButton(
            Transform parent, BugReportFormTheme theme, System.Action onClose)
        {
        ...
        private static void AddLabel(
            Transform parent, string content, BugReportFormTheme theme,
        ...
        private static Sprite? GetLogoSprite()
        {
            ...
            try
            {
                var bytes = System.Convert.FromBase64String(Playloop.Feedback.PlayloopBrandingLogo.PngBase64);
                var tex = new Texture2D(2, 2, TextureFormat.RGBA32, false)
                {
                    wrapMode = TextureWrapMode.Clamp,
                };
        ...
        /// decoded.
        /// </summary>
        private static void AddBrandingBadge(Transform parent, BugReportFormTheme theme)
        {
        ...
        private static void AddCenteredLabel(Transform parent, string content, BugReportFormTheme theme)
        {
        ...
        private static void BuildButton(
            ...
            BugReportFormTheme theme, System.Action onClick)
        {
