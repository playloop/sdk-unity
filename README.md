...
**A still player costs almost nothing.** A sample that repeats the one before it carries no information, so it is not sent. A player who stops to think writes one row, then a keep-alive row every couple of seconds, then a final row when the run ends, instead of one row per tick. Playback holds the position between samples, so the replay looks identical. A row is always kept when anything changes: position, facing, the room, the action bits, the movement axes, or a tracked entity moving. This matters most for games where players pause to plan, which is where a long session would otherwise spend its budget recording a motionless screen.

**Budget.** Each session may send up to `MaxBytesPerSession` (default 2 MB) of Trace data. How long that lasts depends on how much the player actually moves: constant motion at 10 Hz with no entities runs about 90 minutes, while a game with frequent pauses runs considerably longer. At the budget the SDK sends a final chunk marked `budget` and stops; that chunk records where the Trace stopped and why, so the end of the data is never mistaken for the end of the session. Lower `Hz` or track fewer entities for long sessions.
